...
test_data_df = pd.read_csv(os.path.join("data", "fast_answers_mouse_testing.csv"), sep="\t")
...
result_dics = []
k = 1
for d in test_dicts[:1000]:
    q = d["Request"]
    a =  re.sub(r"\xa0", " ", d["ShortAnswerText"])
    t = time.time()
    cls, sgm = predict(q, a)
    d["MouseClass"] = cls
...
result_df.to_csv(os.path.join("data", "fast_answers_checking.csv"), sep="\t")